...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.live_data and not self.sold:
            # With this price the StopLimit order of Backtrader should be mapped to "STOP_LOSS_LIMIT" from Binance.
            # That requires the stop price ("price") to be above the current market price.
            #
            # see https://github.com/binance-exchange/binance-official-api-docs/blob/master/rest-api.md#new-order--trade
            #
            # Attention! Check for yourself if the prices are way above the market price.
            # This way you still have time to cancel the order at the exchange.
            self.order = self.buy(size=2.0, exectype=Order.StopLimit, plimit=77.495, price=77.485)
            self.sold = True
